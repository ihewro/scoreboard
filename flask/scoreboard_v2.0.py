                            ...
                            for item2 in self.instructionList:
                                if item2["issue"] != "" and item2["readOperand"] != "":
                                    if item2["j"] == item["target"] or item2["k"] == item["target"]:
                                        if int(item2["readOperand"]) >= int(cycle) and item2["issue"] != "" and int(
                                                item2["issue"]) < int(item["issue"]):
                                            release_flag = False
...
def printInfo(cycle, insTable, funcUTable, registerTable):
    ...
    if insTable != None:
        ...
        for item in ins:
            print(item["instruction"] + "\t" + item["target"] + "\t" + item["j"] + "\t" + item["k"] + "\t" + item[
                "issue"] + "\t" + item["readOperand"] + "\t" + item["exeComplet"] + "\t" + item["writeResult"])
    # 表2信息
    if funcUTable != None:
        ...
        for item in func:
            print(
                item["name"] + "\t" + item["busy"] + "\t" + item["Op"] + "\t" + item["Fi"] + "\t" + item["Fj"] + "\t" +
                item["Fk"] + "\t" + item["Qj"] + "\t" + item["Qk"] + "\t" + item["Rj"] + "\t" + item["Rk"])
    # 表3信息
    if registerTable != None:
        reg = registerTable.getDict()
        print("Register result status")
        print("F0\tF1\tF2\tF3\tF4\tF5\t")
        print(reg["F0"] + "\t" + reg["F1"] + "\t" + reg["F2"] + "\t" + reg["F3"] + "\t" + reg["F4"] + "\t" + reg[
            "F5"] + "\t")
        print("F6\tF7\tF8\tF9\tF10\tF11\t")
        print(reg["F6"] + "\t" + reg["F7"] + "\t" + reg["F8"] + "\t" + reg["F9"] + "\t" + reg["F10"] + "\t" + reg[
            "F11"] + "\t")
...
def goto_cycle(end_cycle, instructions):
    ...
    result = {
        "cycler": end_cycle,
        "insTable": insTable.getList(),
        "funcUTable": funcUTable.getList(),
        "registerTable": [registerTable.getDict()]
    }
...
def getResult():
    ...
    cycler = int(request.args.get("cycler"))

    # input = "LD F6 34  R2\nLD F2 45  R3\nMULT F0 F2 F4\nSUBD F8 F6 F2\nDIVD F10 F0 F6\nADDD F6 F8 F2"
    print("输入是" + str(input))
    print("循环是" + str(cycler))
    instruction = re.split(' {2}', input)
    print(instruction)
    # 输入指令后更新表1
    insTable = InstructionStatusTable()
    for i in instruction[0:-2]:
        insTable.addInstruction(i)
    return goto_cycle(cycler, instruction)
